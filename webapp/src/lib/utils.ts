...
export function proxyImg (url: string | undefined): string | undefined {
  ...
  if (url.includes('samsungwelstory.com')) return url.replace('http://samsungwelstory.com/', '/img/welstory/')
  if (url.includes('planeatchoice.net')) return url.replace(/https?:\/\/[^/]*planeatchoice\.net\//, '/img/planeat/')
  return url
}
